<template>
	...
	>
		...
		<slot name="prefix" v-else-if="$slots['prefix'] || iconLeft">
			<component v-if="iconLeft" :is="iconLeft" :class="slotClasses" />
		</slot>

		<template v-if="loading && loadingText">{{ loadingText }}</template>
		<template v-else-if="isIconButton && !loading">
			<component v-if="icon" :is="icon" :class="slotClasses" />
		...
		<slot name="suffix">
			<component v-if="iconRight" :is="iconRight" :class="slotClasses" />
		</slot>
	</button>
</template>
...
import type { ButtonProps, Theme, Variant } from "./types";
...
const isIconButton = computed(() => props.icon || slots.icon);
const ariaLabel = computed(() => (isIconButton.value ? props.label : null));
