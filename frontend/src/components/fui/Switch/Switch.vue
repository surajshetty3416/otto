	...
	<div :class="switchGroupClasses">
		<div class="flex flex-col gap-1">
			<div class="flex items-center">
				<component :class="iconClasses" v-if="props.icon" :is="props.icon" />
				<label :class="switchLabelClasses" :for="id">
					{{ props.label }}
				</label>
			</div>

			<span v-if="props.description" class="max-w-xs text-p-sm text-ink-gray-7">{{
				props.description
			}}</span>
		</div>
		<SwitchRoot
...
const switchGroupClasses = computed(() => {
	...
	if (!props.description) {
		...
		classes.push(props.size === "md" ? "px-3 space-x-3.5" : "px-2.5 space-x-2.5");
	}
